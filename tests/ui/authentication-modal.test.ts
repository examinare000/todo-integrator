...
describe('AuthenticationModal', () => {
	...
	describe('core functionality', () => {
		it('should set device code and verification URI', () => {
			// Mock the progressEl and statusEl for this test
			const mockProgressEl = {
				empty: jest.fn(),
				createDiv: jest.fn().mockReturnValue({
					createDiv: jest.fn().mockReturnValue({
			...
			const mockInstructionsEl = {
				show: jest.fn()
			};
			
			// Override the private properties for this test
			(authModal as any).progressEl = mockProgressEl;
			(authModal as any).statusEl = mockStatusEl;
			(authModal as any).instructionsEl = mockInstructionsEl;
