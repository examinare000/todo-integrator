...
describe('SidebarButton', () => {
	...
	describe('updateSyncStatus', () => {
		...
		it('should update status to error with message', () => {
			...
			const mockStatusEl = {
				empty: jest.fn(),
				createSpan: jest.fn().mockReturnValue(mockTextSpan),
				className: ''
			};
			(sidebarButton as any).statusEl = mockStatusEl;
			...
			expect(mockStatusEl.createSpan).toHaveBeenCalledWith('status-icon');
			expect(mockStatusEl.createSpan).toHaveBeenCalledWith('status-text');
			expect(mockTextSpan.textContent).toBe('Error: Custom error message');
		});
	});
